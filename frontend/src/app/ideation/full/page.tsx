...
export default function FullIdeaPage() {
  ...
  const handleEvaluate = async () => {
    ...
    try {
      const result = await evaluateIdea(userInfo?.userId || 'user-123', formData);
          ...
          >
            <div className="mb-6">
              ...
              <p className="mt-2 text-xs" style={{ color: 'var(--color-text-muted)' }}>
                Be as specific as possible. Include your target outcome or benefit.
              </p>
            </div>
        ...
        {evaluation && (
          <div className="space-y-6">
            ...
            <div className="rounded-xl p-8 text-center" style={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
              <h2 className="text-lg mb-2" style={{ color: 'var(--color-text-secondary)' }}>Overall Score</h2>
              <div className={`text-6xl font-bold mb-2 ${getScoreColor(evaluation.scores.overall).split(' ')[0]}`}>
                {formatScore(evaluation.scores.overall)}
              </div>
              <div className={`inline-block px-4 py-2 rounded-full font-semibold ${getScoreColor(evaluation.scores.overall)}`}>
            ...
            <div className="rounded-xl p-8" style={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
              <h3 className="text-xl font-semibold mb-6" style={{ color: 'var(--color-text)' }}>Score Breakdown</h3>
              <div className="grid md:grid-cols-3 gap-6">
                {[
                  { label: 'Virality', score: evaluation.scores.virality, desc: 'Shareability potential' },
                  { label: 'Clarity', score: evaluation.scores.clarity, desc: 'Idea clarity & focus' },
                  { label: 'Competition', score: evaluation.scores.competition, desc: 'Market saturation' },
                ].map((item) => (
                  <div key={item.label} className="text-center">
                    <div className="text-sm mb-1" style={{ color: 'var(--color-text-secondary)' }}>{item.label}</div>
                    <div className={`text-4xl font-bold mb-1 ${getScoreColor(item.score).split(' ')[0]}`}>
                      {formatScore(item.score)}
                    </div>
                    <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{item.desc}</div>
                    <div className="mt-2 rounded-full h-2" style={{ backgroundColor: 'var(--color-border)' }}>
                      <div
            ...
            {/* Improved Version */}
            <div className="rounded-xl p-8" style={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
              <h3 className="text-xl font-semibold mb-4" style={{ color: 'var(--color-text)' }}>Improved Version</h3>
              <div className="p-6 rounded-lg mb-4" style={{ backgroundColor: 'var(--color-surface-hover)', borderLeft: '4px solid var(--color-text)' }}>
                ...
                {evaluation.suggestedHook && (
                  <p className="text-sm flex items-center gap-2" style={{ color: 'var(--color-text-secondary)' }}>
                    <FiTarget className="w-4 h-4" />
                    <span className="font-medium">Hook:</span> {evaluation.suggestedHook}
                  </p>
                )}
              </div>
              <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                <span className="px-3 py-1 rounded-full" style={{ backgroundColor: 'var(--color-surface-hover)' }}>
