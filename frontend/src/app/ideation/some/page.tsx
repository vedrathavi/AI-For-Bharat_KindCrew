...
import { refineIdea, ContentIdea } from '@/lib/api/ideation';
...
export default function SomeIdeaPage() {
  ...
  const handleRefine = async () => {
    if (!formData.roughIdea.trim()) {
      setError('Please enter your rough idea');
      return;
    }
    ...
    try {
      const result = await refineIdea(userInfo?.userId || 'user-123', formData);
      
      if (result.success && result.ideas) {
        setIdeas(result.ideas);
      } else {
        setError(result.error || 'Failed to refine idea');
      }
    } catch (err: unknown) {
              ...
              {ideas.map((idea, index) => (
                <div
                  ...
                  className={`rounded-xl p-6 transition-all cursor-pointer border ${
                    selectedIdea === idea ? 'border-purple-500' : 'border-transparent'
                  }`}
                  style={{
                    backgroundColor: 'var(--color-surface)',
                    borderColor:
                      selectedIdea === idea ? 'var(--color-text)' : 'var(--color-border)',
                  }}
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-xs font-semibold px-3 py-1 rounded-full" style={{ backgroundColor: 'var(--color-surface-hover)', color: 'var(--color-text-secondary)' }}>
                          {idea.platform}
                        </span>
                        <span className="text-xs px-2 py-1 rounded" style={{ backgroundColor: 'var(--color-surface-hover)', color: 'var(--color-text-secondary)' }}>
                          {idea.format}
                        </span>
                      </div>
                      ...
                      {idea.hook && (
                        <p className="text-sm p-3 rounded-lg flex items-center gap-2" style={{ backgroundColor: 'var(--color-surface-hover)', color: 'var(--color-text-secondary)' }}>
                          <FiTarget className="w-4 h-4" />
                          <span className="font-medium">Hook:</span> {idea.hook}
                        </p>
                      )}
                    </div>
                    <div className={`text-3xl font-bold ${getScoreColor(idea.scores.overall)} ml-4`}>
                  ...
                  <div className="grid grid-cols-4 gap-4 text-sm mb-4">
                    <div>
                      <div style={{ color: 'var(--color-text-muted)' }}>Overall</div>
                      <div className={`font-bold text-lg ${getScoreColor(idea.scores.overall)}`}>
                        {formatScore(idea.scores.overall)}
                      </div>
                    </div>
                    <div>
                      <div style={{ color: 'var(--color-text-muted)' }}>Virality</div>
                      <div className={`font-semibold ${getScoreColor(idea.scores.virality)}`}>
                        {formatScore(idea.scores.virality)}
                      </div>
                    </div>
                    <div>
                      <div style={{ color: 'var(--color-text-muted)' }}>Clarity</div>
                      <div className={`font-semibold ${getScoreColor(idea.scores.clarity)}`}>
                        {formatScore(idea.scores.clarity)}
                      </div>
                    </div>
                    <div>
                      <div style={{ color: 'var(--color-text-muted)' }}>Competition</div>
                      <div className={`font-semibold ${getScoreColor(10 - idea.scores.competition)}`}>
