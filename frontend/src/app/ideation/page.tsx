...
export default function IdeationPage() {
  ...
  const paths = [
    {
      ...
      description: 'Get AI-generated content ideas based on your niche and audience',
      icon: FiCompass,
      path: '/ideation/zero',
    },
    {
  ...
  useEffect(() => {
    const loadRecentIdeas = async () => {
      ...
      try {
        const result = await getUserIdeas(userInfo?.userId || 'user-123');
        if (result.success) {
    ...
    <AuthenticatedLayout>
      <div className="w-full max-w-7xl mx-auto overflow-x-hidden">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {paths.map((option) => (
            ...
            >
              ...
              <div className="mt-6 flex items-center font-medium" style={{ color: 'var(--color-text)' }}>
                Get Started
        ...
        >
          ...
          {ideasLoading ? (
            <p style={{ color: 'var(--color-text-secondary)' }}>Loading ideas...</p>
        ...
        >
          ...
          <div className="grid md:grid-cols-3 gap-6 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
            <div>
              <div className="font-medium mb-2" style={{ color: 'var(--color-text)' }}>1. Generate Ideas</div>
              AI analyzes your profile and creates strategic content concepts
            </div>
            <div>
              <div className="font-medium mb-2" style={{ color: 'var(--color-text)' }}>2. Score and Research</div>
              Each idea gets virality, clarity, and competition scores
            </div>
            <div>
              <div className="font-medium mb-2" style={{ color: 'var(--color-text)' }}>3. Select and Proceed</div>
